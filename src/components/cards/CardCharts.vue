<template>
  <div class="q-col-gutter-sm  q-pt-sm">
    <div class="">
      <q-card class="" style="background-color: #38b1c5">
          <div class="text-right q-mr-md q-pt-sm text-white">
            <q-icon size="15px" name="fas fa-sync-alt" @click="updateListener()"/>
          </div>
        <q-card-section class="text-center">
          <!-- <IEcharts style="height: 250px" :option="LineChart2" :resizable="true"/> -->

            <span class="q-ma-lg text-h1 text-bold text-white">
            {{ live }}
            </span>
            ...
            <div class="text-subtitle text-white q-pb-md">
               Conectados agora
            </div>
        </q-card-section>
      </q-card>
...
export default {
  ...
  methods: {
    updateListener () {
     this.$root.$emit('update_listener')   
    }
  },
  mounted () {
    this.$root.$on('update_live', (val)=> {
        this.live = val
        console.log(this.live + 'charts')
